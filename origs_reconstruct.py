...
from lib_scaffold.mosca import MoSca
from lib_scaffold.dynamic_solver import get_dynamic_curves
from lib_scaffold.dynamic_solver import geometry_scf_init
from lib_scaffold.photo_recon_utils import OptimCFG, GSControlCFG
from lib_scaffold.photo_recon import DynReconstructionSolver
from lib_scaffold.static_gs import StaticGaussian
from lib_scaffold.dynamic_solver_utils import (
    round_int_coordinates,
    query_image_buffer_by_pix_int_coord,
)

from origs_viz import viz_main, viz_list_of_colored_points_in_cam_frame
from lite_origs_reconstruct import static_reconstruct
...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    photo_solver.photometric_fit(
        ...
        optimizer_cfg=OptimCFG(
            ...
            # ndg
            lr_diags=getattr(fit_cfg, "photo_lr_diags", 0.00016),
            lr_triangs=getattr(fit_cfg, "photo_lr_triangs", 0.00016),
            lr_Sigma=getattr(fit_cfg, "photo_lr_Sigma", 0.00016),
            lr_mu_t=getattr(fit_cfg, "photo_lr_mu_t", 0.00016),
            lr_mu_ori=getattr(fit_cfg, "photo_lr_mu_ori", 0.00016),
            lr_mu_delta_p=getattr(fit_cfg, "photo_lr_mu_delta_p", 0.00016),
            lr_mu_delta_r=getattr(fit_cfg, "photo_lr_mu_delta_r", 0.00016),
            lr_mu_delta_s=getattr(fit_cfg, "photo_lr_mu_delta_s", 0.00016),
        ),
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    logging.info("=" * 60)
    logging.info(f"Geometric Optimization: M={scaffold.M}, unit={scaffold.spatial_unit}")
    logging.info("=" * 60)
    ...
    if getattr(fit_cfg, "geo_mosca_steps", 1500) > 0:
        scaffold = geometry_scf_init(
            viz_dir=osp.join(log_path, "origs"),
            log_dir=osp.join(log_path, "origs"),
            scf=scaffold,
            cams=cams,
    ...
    # save the scaffold
    os.makedirs(osp.join(log_path, "origs"), exist_ok=True)
    torch.save(scaffold.state_dict(), osp.join(log_path, "origs", "origs.pth"))
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    # load scaffold
    # Support both new (origs) and old (mosca) paths for backward compatibility
    origs_path = osp.join(log_path, "origs", "origs.pth")
    mosca_path = osp.join(log_path, "mosca", "mosca.pth")
    ckpt_path = origs_path if osp.exists(origs_path) else mosca_path
    scaffold = MoSca.load_from_ckpt(torch.load(ckpt_path)).to(device)
...
if __name__ == "__main__":
    ...
    # OriGS Reconstruction Pipeline (4 stages + optional visualization)
    # Stage 1/4: Bundle Adjustment - solve camera poses from tracks and depth
    logging.info("=" * 70)
    logging.info("Stage 1/4: Bundle Adjustment (Static Camera Solving)")
    logging.info("=" * 70)
    static_reconstruct(args.ws, logdir, cfg)

    # Stage 2/4: Photometric Warmup - pre-optimize static background (optional)
    logging.info("=" * 70)
    logging.info("Stage 2/4: Photometric Warmup (Optional Static Background)")
    logging.info("=" * 70)
    photometric_warmup(args.ws, logdir, cfg)

    # Stage 3/4: Scaffold Reconstruction - build motion scaffold from dynamic tracks
    logging.info("=" * 70)
    logging.info("Stage 3/4: Scaffold Reconstruction (Dynamic Motion Field)")
    logging.info("=" * 70)
    scaffold_reconstruct(args.ws, logdir, cfg)

    # Stage 4/4: Photometric Reconstruction - joint optimization of static + dynamic Gaussians
    logging.info("=" * 70)
    logging.info("Stage 4/4: Photometric Reconstruction (Joint Optimization)")
    logging.info("=" * 70)
    photometric_reconstruct(args.ws, logdir, cfg)

    # Visualization (evaluation code removed for demo-only version)
    datamode = getattr(cfg, "mode", "iphone")

    if not args.no_viz and datamode in ["wild"]:
        from origs_viz import viz_main
        viz_main(

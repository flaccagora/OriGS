...
if __name__ == "__main__":
    ...
    logging.debug(f"theta range: [{theta.min():.4f}, {theta.max():.4f}]")
    axis_angle = u * theta[:, None]
    R = axis_angle_to_matrix(axis_angle)
    t = torch.randn(1000, 3)
    t_len = t.norm(dim=-1)
    logging.debug(f"translation length range: [{t_len.min():.4f}, {t_len.max():.4f}]")
    ...
    logging.info(f"Dual quaternion test - angle_error: {angle_error:.6f}, trans_error: {trans_error:.6f}")

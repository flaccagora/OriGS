...
class MoSca(nn.Module):
    def __init__(
        ...
        node_sigma_logit = self.sig_invact(torch.ones(self.T, self.M, 1) * self.init_sigma)  # T,M,1
        self._node_sigma_logit = nn.Parameter(node_sigma_logit)  # T,M,1
    ...
    @property
    def node_sigma(self):
        return self.sig_act(self._node_sigma_logit)  # T,M,1
    ...
    def decremental_topology(
        ...
        for node_id in tqdm(any_change_id.cpu(), dynamic_ncols=True):
    ...
    def update_topology(
        ...
        if len(self._node_grouping.unique()) != len(self.unique_grouping):
            logging.warning("unique_grouping is not consistent with node_grouping")
        ...
        if len(unique_group_id) == 1 or not self.break_topo_between_group:
            ...
            for gid in tqdm(unique_group_id, dynamic_ncols=True):
    ...
    def warp(
        ...
        # get skinning nodes and weights at query time
        sk_ind, sk_mask, query_sk_w, sk_w_sum, sk_ref_node_xyz, sk_ref_node_quat = (
        ...
        ret_xyz, ret_dir = self.__BLEND_FUNC__(
            sk_w=query_sk_w,  # skinning weight
            ...
            dyn_o=dyn_o)  # the target/view leaf's xyz and R
        ...
        if get_multi_node:
            ...
            # Loop to get rotations before and after target_tid
            for dt in range(-half_w, half_w + 1):
                t_neighbor = torch.clamp(target_tid + dt, min=0, max=self.T - 1)  # [N]
                neighbor_xyz, neighbor_quat = self.get_async_knns(t_neighbor, sk_ind)  # [N, K, 3], [N, K, 4]
                xyz_list.append(neighbor_xyz)
                quat_list.append(neighbor_quat)
                tids_list.append(t_neighbor)

            # [N, window, K, 4], can be used externally for smoothing
            sk_dst_node_xyz_smooth = torch.stack(xyz_list, dim=1)  # [N, window, K, 3]
            sk_dst_node_quat_smooth = torch.stack(quat_list, dim=1)  # [N, window, K, 4]
            tids_smooth = torch.stack(tids_list, dim=1)  # [N, window]

            return (ret_xyz, ret_dir, sk_ind, sk_mask,
                    sk_dst_node_xyz, sk_dst_node_quat,
                    sk_dst_node_xyz_smooth, sk_dst_node_quat_smooth, tids_smooth)

        return ret_xyz, ret_dir, sk_ind, sk_mask, sk_dst_node_xyz, sk_dst_node_quat
    ...
    def fast_warp(
        ...
        if get_multi_node:
            ...
            for dt in range(-half_w, half_w + 1):
                t_neighbor = min(max(target_tid + dt, 0), self.T - 1)  # scalar
                neighbor_xyz = self._node_xyz[t_neighbor][sk_ind]       # [N, K, 3]
                neighbor_quat = self._node_rotation[t_neighbor][sk_ind] # [N, K, 4]
                xyz_list.append(neighbor_xyz)
                quat_list.append(neighbor_quat)
                tids_list.append(torch.full((query_xyz.shape[0],), t_neighbor, dtype=torch.long, device=query_xyz.device))
        ...
        return ret_xyz, ret_dir, sk_dst_node_xyz, sk_dst_node_quat
    ...
    def get_skinning_weights(
        ...
        # check
        assert query_xyz.ndim == 2  # N,3

        # select knn nodes and their validity mask; attach_ind: N
        sk_ind = self.topo_knn_ind[attach_ind]  # N,K
        sk_mask = self.topo_knn_mask[attach_ind]  # N,K
        ...
        if isinstance(query_t, int) or query_t.ndim == 0:
            ...
            # Get sigma for same timestep
            sigma_squared = (self.node_sigma[query_t].squeeze(-1))[sk_ind] ** 2  # N,K
        else:
            ...
            # Get sigma for various timesteps
            sigma_squared = self.get_async_knns_buffer(self.node_sigma, query_t, sk_ind).squeeze(-1) ** 2  # N,K
        # sk_ref_node_xyz: [N,K,3]; sk_ref_node_quat: [N,K,4]; sigma_squared: [N,K]

        # exp(-distance) as weight
        sq_dist_to_sk_node = (query_xyz[:, None, :] - sk_ref_node_xyz) ** 2  # N,K,3
        sq_dist_to_sk_node = sq_dist_to_sk_node.sum(-1)  # N,K
        sk_w_un = (
            torch.exp(
                -sq_dist_to_sk_node / (2 * sigma_squared)
            )
            + 1e-6
        )  # N,K

        # filter by validity mask
        sk_w_un = sk_w_un * sk_mask.float()  # N,K
        ...
        sk_w_sum = sk_w_un.sum(-1)  # N
        sk_w = sk_w_un / torch.clamp(sk_w_sum, min=1e-6)[:, None]  # N,K
        return sk_ind, sk_mask, sk_w, sk_w_sum, sk_ref_node_xyz, sk_ref_node_quat
    ...
    def resample_node(self, resample_factor=1.0, use_mask=False, resample_ind=None):
        ...
        new_node_xyz = self._node_xyz[:, resample_ind]  # T,M,3
        new_node_quat = self._node_rotation[:, resample_ind]  # T,M,4
        new_node_sigma_logit = self._node_sigma_logit[:, resample_ind]  # T,M,1
        with torch.no_grad():
    ...
    def append_nodes_pnt(
        ...
        while len(new_node_xyz) > 0:
            ...
            for _t in tqdm(range(self.T), dynamic_ncols=True):
    ...
    def append_nodes_traj(
        ...
        if new_node_sigma_logit is None:
            new_node_sigma_logit = self.sig_invact(
                torch.ones(T, N, 1, device=self.device) * self.init_sigma
            )  # T,N,1
        d = {
        ...
        spec = {"node_xyz": 1, "node_rotation": 1, "node_sigma": 1}
    ...
    # @torch.no_grad()
    # def __compute_R_from_xyz__(self, window_size=3, velocity_eps=1e-4, velocity_weighting=False):
    #     """
    #     Multi-frame Procrustes (centered at t), with velocity-based frame weighting.
    #     """
    #     assert window_size % 2 == 1, "window_size must be odd"
    #     half_w = window_size // 2
    #     M, T = self.M, self.T

    #     # === Step 1: motion-PCA init R0 ===
    #     xyz_seq = self._node_xyz[:window_size]  # [W, M, 3]
    #     xyz_seq = xyz_seq - xyz_seq.mean(dim=0, keepdim=True)
    #     traj = xyz_seq.permute(1, 0, 2).contiguous()  # [M, W, 3]
    #     _, _, V = torch.linalg.svd(traj.double())  # V: [M, 3, 3]
    #     init_node_R = V.float()
    #     new_R_list = [init_node_R]

    #     nn_ind, nn_mask = self.topo_knn_ind, self.topo_knn_mask  # [M, K], [M, K]

    #     for new_tid in tqdm(range(1, T)):
    #         t_start = max(0, new_tid - half_w)
    #         t_end = min(T - 1, new_tid + half_w)
    #         support_ts = list(range(t_start, t_end + 1))
    #         center_t = new_tid

    #         all_src_p, all_dst_q, all_weights = [], [], []
    #         frame_weights = []

    #         # === Step 2: compute per-frame velocity weight ===
    #         if velocity_weighting:
    #             for t in support_ts:
    #                 if t < T - 1:
    #                     v = self._node_xyz[t + 1] - self._node_xyz[t]  # [M, 3]
    #                 else:
    #                     v = self._node_xyz[t] - self._node_xyz[t - 1]  # backward diff
    #                 v_norm = torch.norm(v, dim=-1)  # [M]
    #                 frame_weights.append(v_norm + velocity_eps)  # [M]

    #             # [L, M] → [L, M, 1, 1] for broadcasting
    #             frame_weights = torch.stack(frame_weights, dim=0)  # [W, M]
    #             frame_weights = frame_weights / (frame_weights.sum(dim=0, keepdim=True) + 1e-6)  # [W, M]

    #         for i, t in enumerate(support_ts):
    #             src_xyz = self._node_xyz[t]  # [M,3]
    #             dst_xyz = self._node_xyz[center_t]  # [M,3] fixed target

    #             src_p = src_xyz[nn_ind] - src_xyz[:, None]  # [M,K,3]
    #             dst_q = dst_xyz[nn_ind] - dst_xyz[:, None]  # [M,K,3]
    #             w = nn_mask.clone()  # [M,K]
    #             w = w / w.sum(dim=-1, keepdim=True)  # [M,K]

    #             # Multiply by frame velocity weight
    #             if velocity_weighting:
    #                 fw = frame_weights[i].unsqueeze(-1)  # [M,1]
    #                 w = w * fw  # [M,K] with temporal reweighting

    #             src_centroid = (src_p * w[..., None]).sum(1)  # [M,3]
    #             dst_centroid = (dst_q * w[..., None]).sum(1)  # [M,3]

    #             src_p = src_p - src_centroid[:, None]
    #             dst_q = dst_q - dst_centroid[:, None]

    #             all_src_p.append(src_p)
    #             all_dst_q.append(dst_q)
    #             all_weights.append(w)

    #         src_p_stack = torch.cat(all_src_p, dim=1)  # [M, K*W, 3]
    #         dst_q_stack = torch.cat(all_dst_q, dim=1)  # [M, K*W, 3]
    #         w_stack = torch.cat(all_weights, dim=1)    # [M, K*W]

    #         W = torch.einsum("nki,nkj->nkij", src_p_stack, dst_q_stack)  # [M,K*W,3,3]
    #         W = W * w_stack[:, :, None, None]
    #         W = W.sum(1)  # [M,3,3]

    #         U, s, V = torch.svd(W.double())
    #         R_tmp = torch.einsum("nij,nkj->nik", V, U)
    #         det = torch.det(R_tmp)
    #         dia = torch.ones(len(det), 3, device=det.device)
    #         dia[:, -1] = det
    #         Sigma = torch.diag_embed(dia).double()  # [M,3,3]
    #         V = torch.einsum("nij,njk->nik", V, Sigma)
    #         R_star = torch.einsum("nij,nkj->nik", V, U)  # [M,3,3]

    #         next_R = torch.einsum("nij,njk->nik", R_star.double(), new_R_list[-1].double())
    ...
    def __compute_R_from_xyz__(self):
        ...
        xyz_seq = self._node_xyz[:5]  # [W, M, 3]
        xyz_seq = xyz_seq - xyz_seq.mean(dim=0, keepdim=True)  # [W, M, 3]
        traj = xyz_seq.permute(1, 0, 2).contiguous()  # [M, W, 3]
        _, _, V = torch.linalg.svd(traj.double())  # V: [M, 3, 3]
        init_node_R = V.float()  # [M, 3, 3]

        new_R_list = [init_node_R]
        nn_ind, nn_mask = self.topo_knn_ind, self.topo_knn_mask  # [M, K], [M, K]
        for new_tid in tqdm(range(1, self.T), dynamic_ncols=True):
    ...
    def resample_time(self, new_tids, new_node_certain, mode="linear"):
        ...
        new_xyz_list, new_quat_list, new_sigma_list = [], [], []
        for new_t in tqdm(new_tids, dynamic_ncols=True):
            # get left node
            left_t = self._t_list[self._t_list <= new_t].max()
            left_ind = (self._t_list == left_t).float().argmax()
            assert left_ind >= 0 and left_ind < self.T
            l_xyz = self._node_xyz[left_ind]
            l_quat = self._node_rotation[left_ind]
            l_sigma = self._node_sigma_logit[left_ind]

            if left_t == new_t:
                new_xyz_list.append(l_xyz)
                new_quat_list.append(l_quat)
                new_sigma_list.append(l_sigma)
                continue
            ...
            r_xyz = self._node_xyz[right_ind]
            r_quat = self._node_rotation[right_ind]
            r_sigma = self._node_sigma_logit[right_ind]

            if right_t == new_t:
                new_xyz_list.append(r_xyz)
                new_quat_list.append(r_quat)
                new_sigma_list.append(r_sigma)
                continue
            ...
            new_sigma = l_sigma * l_w + r_sigma * r_w
            if mode == "dq":
...
def __identify_spatial_unit_from_curves__(xyz, mask, K=10):
    T = len(xyz)
    dist_list = []
    for t in tqdm(range(T), dynamic_ncols=True):
        _p = xyz[t][mask[t]]

...
class DynReconstructionSolver:
    ...
    def identify_fg_mask_by_nearest_curve(
        ...
        with torch.no_grad():
            fg_mask_list = []
            for query_tid in tqdm(range(s2d.T), dynamic_ncols=True):
    ...
    def compute_normals_for_s2d(
        ...
        normal_map_list = []
        new_mask_list = []
        for t in tqdm(range(s2d.T), dynamic_ncols=True):
            dep = s2d.dep[t].clone()
    ...
    def get_static_model(
        ...
        # get params, involving n_attach to mitigate redundancy
        mu_init, q_init, s_init, o_init, rgb_init, id_init, _ = (  # _ is time ind, s gs don't need it
            fetch_leaves_in_world_frame(
                cams=cams,
                n_attach=n_init,
    ...
    def get_dynamic_model(
        ...
        if topo_th_ratio is not None:
            ...
            logging.info(f"Reset SCF topo th ratio from {old_th_ratio} to {scf.topo_th_ratio}")

        # init d_model (mainly about scf and empty leaf attr including ref/query time tid)
        ...
        # append the leaves based on params fetched
        ...
        for tid in tqdm(unique_tid, dynamic_ncols=True):
            t_mask = time_init == tid
            d_model.append_new_gs(  # get attched scf nodes at time tid by knn
                optimizer,
                tid=tid,  # ref_time, the time when the leaf is init
                mu_w=mu_init[t_mask],
    ...
    def photometric_fit(
        ...
        # init uv for 2d flow and track computation
        ...
        if d_flag:
            ...
            s_model.get_cate_color(  # category color
        ...
        for step in tqdm(range(total_steps), dynamic_ncols=True):
            # control the weight correction
            ...
            # correspondance loss execution flag
            corr_exe_flag = (
                corr_flag
            ...
            # get view and index list
            view_ind_list = np.random.choice(cams.T, n_frame, replace=False).tolist()

            # compute flow
            if corr_exe_flag:
                # select another destination index different than the view_ind_list
            ...
            # init rgb and correspondance render list
            ...
            # for each opt step, iterate over target/view t list
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                # the dst_ind is different from view_ind if corr_exe_flag is true
                # these are for the correspondance loss
                dst_ind = corr_dst_ind_list[_inner_loop_i]
                flow_flag = corr_flow_flag_list[_inner_loop_i]

                # static model 5 params for rendering
                gs5 = [list(s_model())]
                ...
                # dynamic model 5 params for rendering
                if d_flag:
                    gs5.append(list(d_model(view_ind)))  # forward, view/target time index
                ...
                # render
                render_dict = render(
                    gs5,  # render s and d gs together
                    s2d.H,
                    s2d.W,
                    cams.K(s2d.H, s2d.W),
                    cams.T_cw(view_ind),
                ...
                render_dict_list.append(render_dict)  # rgb, corr, etc

                # compute rgb loss
                rgb_sup_mask = s2d.get_mask_by_key(sup_mask_type)[view_ind]  # supervision mask
                _l_rgb, _, _, _ = compute_rgb_loss(
                    s2d.rgb[view_ind].detach().clone(), render_dict, rgb_sup_mask)
                loss_rgb = loss_rgb + _l_rgb

                # compute dep loss
                dep_sup_mask = rgb_sup_mask * s2d.dep_mask[view_ind]  # supervision mask
                ...
                if d_flag and lambda_mask > 0.0:
                    ...
                    s_cate_sph, s_gid2color = s_model.get_cate_color(  # category color
            ...
            # compute vel and acc regularization loss
            if (
                lambda_vel_xyz_reg > 0.0
            ...
            # compute small w regularization loss
            if d_flag:
            ...
            delta_p_grad = d_model._mu_delta_p.grad.detach()  # [N, 3]
            # Sigma_res_cond_grad = d_model._Sigma_res_cond.grad.detach()  # [N, 50]
            ...
            # d_model to s_model transfer [step 1] copy the d_gs5
            ...
            # static + dynamic gs control (densify, prune, reset)
            if (
            ...
            # d_model to s_model transfer [step 2] append to static model
            if dynamic_to_static_transfer_flag:
                s_model.append_gs(optimizer_static, *trans_d_gs5, new_group_id=None)

            # dynamic gs gradient based densification
            ...
            # record small w and track loss for logging
    ...
    def render_all(self, cams: MonocularCameras, s_model=None, d_model=None):
        ...
        s_gs5 = s_model()
        for t in tqdm(range(cams.T), dynamic_ncols=True):
            gs5 = [s_gs5]
            if d_model is not None:

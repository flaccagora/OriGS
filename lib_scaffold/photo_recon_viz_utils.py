...
from lib_scaffold.gs_utils.loss_helper import (
    compute_rgb_loss,
    compute_dep_loss,
    compute_normal_loss,
)
from lib_scaffold.scaffold_utils.viz_helper import viz_curve
...
def make_video(src_pattern, dst):
    ...
    image_fns.sort()
    if len(image_fns) == 0:
        logging.warning(f"no image found in {src_pattern}")
        return
    frames = []

...
def apply_gs_control(
    ...
    for render_dict in render_list:
        if first_N is not None:
            ...
            if ndg_grad is not None:
                ndg_grad = ndg_grad[:first_N]
        ...
        if record_flag:
            ...
            if ndg_grad is not None:
                model.record_ndg_grad(ndg_grad, visib)
    if (
...
class OptimCFG:
    def __init__(
        ...
        # ndg
        self.lr_diags = lr_diags
        self.lr_triangs = lr_triangs
        self.lr_Sigma = lr_Sigma
        self.lr_mu_t = lr_mu_t
        self.lr_mu_ori = lr_mu_ori
        self.lr_mu_delta_p = lr_mu_delta_p
        self.lr_mu_delta_r = lr_mu_delta_r
        self.lr_mu_delta_s = lr_mu_delta_s
        # cam
    ...
    def summary(self):
        ...
        logging.info(f"OptimCFG: lr_diags={self.lr_diags}")
        logging.info(f"OptimCFG: lr_triangs={self.lr_triangs}")
        logging.info(f"OptimCFG: lr_Sigma={self.lr_Sigma}")
        logging.info(f"OptimCFG: lr_mu_t={self.lr_mu_t}")
        logging.info(f"OptimCFG: lr_mu_ori={self.lr_mu_ori}")
        logging.info(f"OptimCFG: lr_mu_delta_p={self.lr_mu_delta_p}")
        logging.info(f"OptimCFG: lr_mu_delta_r={self.lr_mu_delta_r}")
        logging.info(f"OptimCFG: lr_mu_delta_s={self.lr_mu_delta_s}")
    ...
    def get_dynamic_lr_dict(self):
        return {
            ...
            "lr_diags": self.lr_diags,
            "lr_triangs": self.lr_triangs,
            "lr_Sigma": self.lr_Sigma,
            "lr_mu_t": self.lr_mu_t,
            "lr_mu_ori": self.lr_mu_ori,
            "lr_mu_delta_p": self.lr_mu_delta_p,
            "lr_mu_delta_r": self.lr_mu_delta_r,
            "lr_mu_delta_s": self.lr_mu_delta_s,
        }
...
def fetch_leaves_in_world_frame(
    ...
    # for each time t
    for t in tqdm(t_list, dynamic_ncols=True):
        # get the mask, depth
...
def error_grow_dyn_model(
    ...
    for grow_tid in tqdm(grow_tids, dynamic_ncols=True):
...
def identify_rendering_error(
    ...
    error_list = []
    for t in tqdm(range(d_model.T), dynamic_ncols=True):  # ! warning, d_model.T may smaller than cams.T
